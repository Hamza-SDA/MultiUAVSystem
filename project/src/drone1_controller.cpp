...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        if ( mission_progress > 0 && rel_z > 2 && (ros::Time::now() - last_request > ros::Duration(15.0)))
        {
            pose.pose.position.x = current_x - rel_x;
            pose.pose.position.y = current_y - rel_y;
            pose.pose.position.z = drone_change;
            mission_progress++;
            ROS_INFO("Drone 1 Position changed");
        }
        ...
        //    mission_progress++;
        //    ROS_INFO("Drone 1 Landing");
        //    break;
        //}
        
        // ROS_INFO("%.2f, %.2f", drone_change, rel_z);

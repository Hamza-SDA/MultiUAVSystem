...
int main(int argc, char **argv)
{
    ...
    state_sub = nh.subscribe<mavros_msgs::State>("/uav2/mavros/state", 10, state_cb);				// State Subscriber
    mavros_pos = nh.subscribe<geometry_msgs::PoseStamped>("/uav2/mavros/vision_pose/pose", 10, mavros_pos_cb);	// Mavros vehicle position Subscriber
    ...
    local_pos_pub = nh.advertise<geometry_msgs::PoseStamped>("/uav2/mavros/setpoint_position/local", 10);		// Local vehicle position Publisher
    
    //
    global_pos_pub = nh.advertise<uas_offboard_planner::Drone_pos>("/uav2/global_pos", 10);
    
    arming_client = nh.serviceClient<mavros_msgs::CommandBool>("/uav2/mavros/cmd/arming");				// ROS arming service
    set_mode_client = nh.serviceClient<mavros_msgs::SetMode>("/uav2/mavros/set_mode");				// ROS set mode service
